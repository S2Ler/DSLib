...
!!! 10. init -> start -> startOnRunLoopThreadThread -> cancel -> finish -> cancelOnRunLoopThreadThread -> dealloc
    ...
    10. In this case the -cancelOnRunLoopThread loses the race with finish, but that's OK 
        ...
        finished.
    11. Cancellation after finishing still sets isCancelled but has no impact
...
/* Change the state of the operation, sending the appropriate KVO
notifications.
 */
...
- (void)finishWithError:(NSError *)error
{
  ...
  [self setState:kQRunLoopOperationStateFinished];
}
...
- (void)start
{
  ...
  [self setState:kQRunLoopOperationStateExecuting];
...
- (void)cancel
{
  ...
  @synchronized (self) {
    ...
    runCancelOnRunLoopThread
      = oldValue == NO && [self state] == kQRunLoopOperationStateExecuting;
  }
